...
ui <- fluidPage(
    ...
    tabsetPanel(
       ...
       tabPanel("Reformed (9 - 1): 2021 - 2018",

                fluidRow(

                    column(3,
                           tags$br(),
                           selectInput("InputYear2", "Year", c(2021:2018))
                    ),
                    column(8,
...
server <- function(input, output, session) {
    ...
    output$Legacy <- renderPlotly({
        ...
        plot_width<- STEMonly %>% filter(Year == input$InputYear) %>%
    ...
    output$Reformed <- renderPlotly({
        ...
        plot_width<- STEMonly %>% filter(Year == input$InputYear2) %>%
            group_by(TypeSubject) %>%
            summarise(num = length(unique(Subject))) %>%
            ungroup() %>%
            mutate(width_pct = num/sum(num))

        STEMonly %>% filter(Year == input$InputYear2) %>%
            split(.$TypeSubject) %>%
            purrr::map(function(x) {
                ggplot(data=x,aes(x=Grades,y=Subject,fill=Percent,
                                  text = paste0("Candidates: ", round(Rawnum))))+
